...
- **Withdrawal-validation sandbox** — rota `POST /billing/webhooks/asaas/sandbox/withdrawal-validation` + `billingWebhookService.validateAsaasSandboxWithdrawal`. Mesma postura (404 fora de sandbox; verifica `asaas-access-token` timing-safe→401; IP-rate-limited; metadata-only). **DEFAULT-DENY:** v0.1 sempre responde `200 { status: "REFUSED", refuseReason }` — contrato documentado pelo Asaas para recusa; **body nunca é lido/hasheado/logado/salvo**; não toca `billingService`/assinatura/soft-lock/financeiro da clínica/pacientes. Comportamento final a confirmar contra sandbox real.
