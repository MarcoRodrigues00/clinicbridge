...
- **Ajustes de QA (continuação 3.17):** (1) Agenda — badge classificando a data selecionada (`dayKind` via comparação de YYYY-MM-DD local): **Hoje** (cyan), **Amanhã** (teal), **Ontem** (âmbar), **Data selecionada** (neutro); botão "Hoje" ganha estado ativo quando a data é hoje — resolve a confusão de "parecer Hoje" ao navegar dias. (2) Telas de login/cadastro: `AuthAside` reescrito (título "Gestão administrativa segura para clínicas" + 5 bullets: Pacientes organizados, Agenda administrativa, Sessão protegida, Migração e exportação, Não é prontuário clínico — removido o bullet desatualizado "Upload CSV/XLSX em breve"); subtítulos de `RegisterPage`/`LoginPage` ampliados (pacientes/importação-exportação/duplicados/agenda/auditoria), sem prometer prontuário/clínico/produção/compliance. typecheck+build OK.
